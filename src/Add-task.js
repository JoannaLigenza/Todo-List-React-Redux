...
class AddTask extends Component {
    state = { task: "Add new task", id: 5, list: "Default", date:"", time: "", priority: "None", color: "", moveTaskStyle: false }
    handleChange = (e) => {
    ...
    addTask = (e) => {
        ...
        this.setState( {id: id})
        this.props.addTask(this.state.task, this.state.id, this.state.list, this.state.date, this.state.time, this.state.priority, this.state.color, this.state.moveTaskStyle);
...
const mapDispatchToPost = (dispatch) => {
    return {
        addTask: (task, id, list, date, time, priority, color, style) => { dispatch( { type: 'ADD_TASK', task: {task: task, id: id, checked: false, edit: false, list: list, date: date, time: time, priority: priority, color: color, moveTaskStyle: style}} ) },
